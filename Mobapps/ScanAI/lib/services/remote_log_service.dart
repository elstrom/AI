...
class RemoteLogService {
  ...
  static const String _source = 'scanai';
  ...
  void initialize() {
    ...
    if (AppConstants.isDebugMode) {
      debugPrint(
          '[RemoteLog] Initialized with endpoint: ${AppConstants.remoteLogEndpoint}');
    }
  }
  ...
  void _startFlushTimer() {
    ...
    _flushTimer = Timer.periodic(
      const Duration(milliseconds: AppConstants.remoteLogFlushIntervalMs),
      (_) => _flush(),
    );
  }
  ...
  void log(String level, String message) {
    ...
    if (_buffer.length >= AppConstants.remoteLogBufferSize) {
      _flush();
    }
  }
  ...
  Future<void> _flush() async {
    ...
    try {
      ...
      final response = await _client
          .post(
            Uri.parse(AppConstants.remoteLogEndpoint),
